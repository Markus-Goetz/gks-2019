...
\begin{columns}
    ...
    \begin{column}{0.48\textwidth}
        \begin{figure}
            ...
            \node at (axis cs:1.30,0.8) [presentblue] {$lr\nabla_{w}J(w)$};
...
\begin{itemize}
\item Cumbersome to keep track of weights $w$ and bias $b$ simultaneously
...
\begin{columns}
    \begin{column}{0.48\textwidth}
        \begin{itemize}
            ...
            \medskip
            $\nabla_{w}J(w)=(\hat{y}-y)*y^2*e^{-wx}*x$
...
\begin{frame}
\frametitle{Exercise 1: Logistic Regression}
...
\begin{columns}
    \begin{column}{0.48\textwidth}
        \begin{itemize}
            ...
            \medskip
            $\nabla_{w}J(w)=(\hat{y}-y)*y^2*e^{-wx}*x$
            \item \textbf{Train:} $w_{i+1}=w_{i}-lr\nabla_{w}J(w)$
...
\begin{columns}
    ...
    \begin{column}{0.32\textwidth}
        ...
        \textbf{AND-gate}\\
        \medskip
        $\hat{y}=\mathbf{h_1} + \mathbf{h_2} - 1.5$
        \end{onlyenv}
    \end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    \item Randomly turn of neurons and connection, e.g. $p(drop)=0.3$
    \item Equivalent to network regularization (proof omitted)
    \item Implementation: set passed input to zero
\end{itemize}
...
\begin{frame}
    \frametitle{Exercise 2: FNN MNIST Image Classification}
...
\begin{columns}
    \begin{column}{0.58\textwidth}
        \begin{itemize}
            ...
            \item $(f*g)[n]=\sum_{m=-K}^{K}f[m]g[n-m]$
            \item \textbf{Filter size K:} window size of convolution kernel
...
\begin{frame}
\frametitle{Exercise 3: CNN MNIST Image Classification}
...
%
%\begin{frame}
%    \frametitle{Exercise 4: Abalone Age Regression Analysis}
%    \begin{figure}
%        \centering
